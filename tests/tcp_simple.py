    ...
    def _process_iteration(self, iteration_idx, data_len, unique_data,
        ...
        try:
            ...
            if unique_data:
                ...
                print(f"Iteration {iteration_idx+1}: Using shared buffer, MD5: "
                      f"{original_md5}")

            client_socket.send(data_bytes)
            print(f"Iteration {iteration_idx+1}: Sent {len(data_bytes)} bytes")

            # Receive echo response
            response = self._receive_response(client_socket, len(data_bytes))
            print(f"Iteration {iteration_idx+1}: Received {len(response)} bytes")

            # Calculate MD5 of received data
            received_md5 = calculate_md5(response)
            print(f"Iteration {iteration_idx+1}: Received MD5: {received_md5}")

            # Verify checksum
            checksum_match = original_md5 == received_md5

            if checksum_match:
                print(f"Iteration {iteration_idx+1}: ✓ MD5 checksum verified "
                      f"correctly")
            else:
                print(f"Iteration {iteration_idx+1}: ✗ MD5 checksum mismatch!")
            ...
            print(f"Iteration {iteration_idx+1}: Unexpected error: {general_error}")
            return {'success': False, 'checksum_match': False}
